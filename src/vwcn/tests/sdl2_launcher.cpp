...
void launcher_sdlError(const char *mes)
{
	fprintf(stderr, "%s: %s\n", mes, SDL_GetError());
	exit(1);
}
...
void launcher_Init()
{
	...
	if(SDL_Init(SDL_INIT_EVERYTHING) < 0)
		launcher_sdlError("Could not initialize SDL");
	...
	if(window == NULL)
		launcher_sdlError("Could not create window");
	...
	if(glContext == NULL)
		launcher_sdlError("Could not create the OpenGL context");
...
void launcher_DeleteGLTrash()
{
	glDeleteVertexArrays(1, &VAO);
	glDeleteBuffers(1, &VBO);
}

void launcher_DeleteSDLTrash()
{
...
{
	launcher_Init(); // Init SDL2 Glad
	
	launcher_PreLoop(); // To do before loop (display graphics card info)
	
	launcher_OpenGLSet(); // set up OpenGL
	...
	launcher_DeleteGLTrash();
	launcher_DeleteSDLTrash();
	SDL_Quit();
